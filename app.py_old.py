...
def main() -> None:
    ...
    try:
        ...
        if not tx_df.empty:
            ...
            total_realized_krw = krw_sell["realized_pnl"].sum()
            total_realized_usd = usd_sell["realized_pnl"].sum()
            ...
            st.caption("매도 거래가 없습니다.")
    except Exception as e:
        st.error(f"실현손익 계산 오류: {e}")

    with st.expander("시작 정리 가이드", expanded=False):
        st.markdown("""
        1. **테스트 거래 정리**: 거래기록 관리에서 테스트 거래 일괄 삭제
        2. **현재 계좌 기준 맞추기**: 필요하면 전체 거래기록 초기화 후 보유종목 수동 조정으로 현재 수량/평단 입력
        3. **액면분할 반영**: LS ELECTRIC 같은 종목은 보유종목 조정 도구에서 분할 비율 적용
        4. **Render URL 반영**: 로컬에서만 바뀌면 Git commit/push 후 Render 재배포 필요
        """)
